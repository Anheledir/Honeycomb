...
public class CommandHandler
{
    ...
    private readonly static ILogger _staticLogger = Program.ServiceProvider.GetRequiredService<ILogger>();
    ...
    {
        var methodMap = _cache.GetOrCreate("methodMap", entry =>
        {
        ...
        try
        {
            if (methodMap?.TryGetValue(commandName, out var method) ?? false)
            {
                ...
                var convertedArguments = arguments == null ? Array.Empty<object>() : ConvertArguments(arguments, argumentTypes);
